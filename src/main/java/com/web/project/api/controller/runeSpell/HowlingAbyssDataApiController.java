...
import com.web.netflix.dto.DataEntry;
import com.web.netflix.dto.Selections;
import com.web.netflix.dto.SummonerSpellSetWinRate;


@Controller
public class HowlingAbyssDataApiController {

	// 참고로 perk = rune
	private String defaultFilePath;

    public HowlingAbyssDataApiController() {
    ...
    public String getRuneData(Model model,
    	...
    	try {
            ...
            String filePath = defaultFilePath + tier + "\\HowlingAbyss_data.json";
            System.out.println(defaultFilePath + tier + "\\HowlingAbyss_data.json");
                        
            String rawData = Files.readString(Paths.get(filePath));
            
            List<DataEntry> data = parseJson(rawData);            
           
            List<DataEntry> filteredData = filterData(data, tier, championId);
            ...
            model.addAttribute("primaryPerk2", primaryStylePerks234.get(0));
            model.addAttribute("primaryPerk3", primaryStylePerks234.get(1));
            model.addAttribute("primaryPerk4", primaryStylePerks234.get(2));
            ...
            e.printStackTrace();
            System.out.println("no howlingabyss");
        }
    ...
    private List<DataEntry> filterData(List<DataEntry> data, String tier, int championId) {
        return data.stream()
                .filter(entry -> entry.getIndividualPosition().equals("Invalid")
                        && entry.getChampionId() == championId)
