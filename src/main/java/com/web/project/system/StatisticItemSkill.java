...
public class StatisticItemSkill {
	...
	public static List<ItemStatistics> filterStatistic(List<ItemStatistics> data, String tier, String position,
			String championName) {
	...
	public static List<ItemListStatistics> filterStatistic3(List<ItemListStatistics> data, String tier, String position,
			String championName) {
	...
	public static List<SkillStatistics> filterStatistic2(List<SkillStatistics> data, String tier, String position,
			String championName) {
	...
	public static List<ItemListStatistics> caculateItems(List<ItemListStatistics> filteredStatistic) {
		...
		List<ItemListStatistics> Items = filteredStatistic.stream()
				.filter(winrate -> winrate.getWinRate() <1)
				.filter(stat -> stat.getPickCount() > 1)
				.limit(1)
				.collect(Collectors.toList());
		return Items;
	}
	...
	public static List<SkillStatistics> caculateSkills(List<SkillStatistics> filteredStatistic) {
		filteredStatistic.sort(Comparator.comparingDouble(SkillStatistics::getWinRate).reversed());
		List<SkillStatistics> mostSkill = filteredStatistic.stream().filter(stat -> stat.getPickCount() > 5).limit(1)
				.collect(Collectors.toList());
		return mostSkill;
	}
